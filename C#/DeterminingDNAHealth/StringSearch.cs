...
namespace DeterminingDNAHealth
{
	...
	public class StringSearch : IStringSearchAlgorithm
	{
		...
		public Dictionary<string, int> FindKeywordOccurency(string text)
		{
			Dictionary<string, int> keywordOccurency = new Dictionary<string, int>();
			...
			while (index < text.Length)
			{
				...
				foreach (string found in ptr.Results)
                {
					keywordOccurency.TryGetValue(found, out int count);
					keywordOccurency[found] = ++count;
				}
